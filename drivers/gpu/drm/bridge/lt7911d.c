...
{
	struct lt7911d_bridge *lt7911d_bridge =
		drm_connector_to_lt7911d_bridge(connector);
	struct edid *edid;
	int ret;

	if (lt7911d_bridge->next_bridge->ops & DRM_BRIDGE_OP_EDID) {
		edid = drm_bridge_get_edid(lt7911d_bridge->next_bridge,
					   connector);
		if (!edid)
			DRM_INFO(
...
static const struct drm_connector_helper_funcs
	lt7911d_bridge_con_helper_funcs = { .get_modes =
						    lt7911d_bridge_get_modes };

static enum drm_connector_status
lt7911d_bridge_detect(struct drm_connector *connector, bool force)
{
	struct lt7911d_bridge *lt7911d_bridge =
		drm_connector_to_lt7911d_bridge(connector);
...
// 		drm_bridge_to_lt7911d_bridge(bridge);
// 	int ret;

// 	ret = drm_bridge_attach(bridge->encoder, &lBridge->bridge,
...
// 	if (!bridge->encoder) {
// 		DRM_ERROR("No encoder attached to bridge\n");
// 		return -ENODEV;
// 	}

// 	drm_connector_helper_add(&lBridge->connector,
// 				 &lt7911d_bridge_con_helper_funcs);

// 	ret = drm_connector_init_with_ddc(bridge->dev,
// 					  &lBridge->connector,
// 					  &lt7911d_bridge_con_funcs,
// 					  lBridge->info->connector_type,
// 					  lBridge->next_bridge->ddc);
// 	if (ret) {
// 		DRM_ERROR("Failed to initialize connector\n");
// 		return ret;
// 	}

// 	drm_connector_attach_encoder(&lBridge->connector,
// 				     bridge->encoder);
